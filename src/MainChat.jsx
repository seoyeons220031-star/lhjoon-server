          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <div className="flex gap-1">
              {Object.keys(themes).map(k => (
                <button type="button" key={k} onClick={() => setCurrentThemeKey(k)} className={`text-[10px] px-1.5 py-0.5 rounded border ${currentThemeKey === k ? 'bg-white font-bold text-black border-white' : 'opacity-60 bg-zinc-800 text-white border-zinc-700'}`}>{themes[k].name.split(' ')[0]}</button>
              ))}
            </div>
          </div>
          <div className="flex items-center justify-between text-[11px]">
            <span className="font-bold text-gray-300">🔒 4자리 앱 잠금 활성화</span>
            <button type="button" onClick={() => { const p = prompt('새로운 PIN 4자리를 설정하세요:'); setSavedPin(p || ''); if (p) localStorage.setItem('lhjoon_app_pin', p); else localStorage.removeItem('lhjoon_app_pin'); }} className="bg-zinc-800 border border-zinc-700 px-2 py-0.5 rounded font-bold text-xs text-white hover:bg-zinc-700">{savedPin ? '변경' : '설정하기'}</button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-2 py-3 space-y-4 bg-zinc-950/30">
              ...
              {rooms.map(room => (
                <div key={room.id} onClick={() => setActiveRoomId(room.id)} className={`p-2.5 rounded-xl cursor-pointer flex items-center justify-between group transition-all ${room.id === activeRoomId ? 'bg-zinc-900 shadow-md font-bold border-l-4 border-blue-500 text-white' : 'hover:bg-zinc-900/50 text-gray-300'}`}>
                  ...
                  <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity ml-1">
                    <button type="button" onClick={(e) => { e.stopPropagation(); setRooms(rooms.map(r => r.id === room.id ? { ...r, isPinned: !r.isPinned } : r)); }} className="text-[10px] bg-zinc-800 border border-zinc-700 px-1 rounded text-white">핀</button>
                    <button type="button" onClick={(e) => { e.stopPropagation(); setRooms(rooms.map(r => r.id === room.id ? { ...r, isMuted: !r.isMuted } : r)); }} className="text-[10px] bg-zinc-800 border border-zinc-700 px-1 text-white">{room.isMuted ? '소리' : '무음'}</button>
                    <button type="button" onClick={(e) => { e.stopPropagation(); handleExitOrDeleteRoom(room.id); }} className="text-[10px] bg-red-950 text-red-400 border border-red-900 p-1 rounded hover:bg-red-900"><Trash2 size={11} /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="border-t border-zinc-800 pt-3">
            ...
            <div className="flex gap-1.5 px-2 mb-2">
              ...
              <button type="button" onClick={handleAddFriendSubmit} className="text-xs bg-white text-black px-2 rounded-lg font-bold shrink-0 hover:bg-gray-200">추가</button>
            </div>
            <div className="space-y-1 px-1">
              {friends.map((f, idx) => (
                <div key={idx} className="flex items-center justify-between text-xs p-1.5 bg-zinc-900/50 rounded-lg border border-zinc-800/40">
                  ...
                  <button type="button" onClick={() => setFriends(friends.map((friend, i) => i === idx ? { ...friend, isBlocked: !friend.isBlocked } : friend))} className={`text-[10px] px-1 border rounded ${f.isBlocked ? 'bg-red-600 border-red-700 text-white' : 'bg-zinc-800 border-zinc-700 text-gray-300'}`}>{f.isBlocked ? '차단됨' : '차단'}</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col h-full bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden relative">
        ...
        <div className="h-16 border-b border-zinc-800 bg-zinc-900 px-4 flex items-center justify-between text-white z-10">
          <div className="flex items-center space-x-2.5">
            {!isSidebarOpen && (
              <button type="button" onClick={() => setIsSidebarOpen(true)} className="p-2 mr-1 text-white bg-zinc-800 rounded-xl hover:bg-zinc-700" title="사이드바 열기">
                <Menu size={18} />
              </button>
            )}
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <button type="button" onClick={() => { const n = prompt('새로운 방 이름을 작성해 주세요:'); if (n?.trim()) setRooms(rooms.map(r => r.id === currentRoom.id ? { ...r, name: n.trim() } : r)); }} className="text-[11px] text-blue-400 underline hover:text-blue-300 font-bold">이름 수정</button>
              </div>
              <p className="text-[11px] text-gray-300">참여자: {currentRoom?.members?.join(', ')}</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button type="button" onClick={() => { const f = prompt('초대할 친구 이름:'); if(f?.trim()) setRooms(rooms.map(r => r.id === currentRoom.id ? { ...r, members: [...r.members, f.trim()] } : r)); }} className="flex items-center gap-1 bg-white text-black font-bold px-3 py-1.5 rounded-xl text-xs hover:bg-gray-200"><UserPlus size={12} /> 친구 초대</button>
          </div>
        </div>
              ...
              <div className="flex items-end space-x-1.5 space-x-reverse max-w-md">
                ...
                <div className="text-right text-[10px] text-gray-400 font-medium min-w-[50px]">
                  ...
                  <div className="opacity-0 group-hover:opacity-100 flex items-center gap-1 mt-1 transition-opacity justify-end">
                    <button type="button" onClick={() => setReplyTarget(msg)} className="text-gray-400 hover:text-white" title="답장"><Reply size={11} /></button>
                    ...
                    {msg.isMe && (
                      <div className="flex gap-1 ml-1 bg-zinc-900/90 px-1.5 py-0.5 rounded border border-zinc-700 text-[9px] font-bold">
                        <button type="button" onClick={() => { setEditingMessageId(msg.id); setEditMessageText(msg.content.replace(' (수정됨)','')); }} className="text-blue-400 hover:underline">수정</button>
                        <button type="button" onClick={() => handleExecuteDeleteMessage(msg.id)} className="text-red-400 hover:underline">삭제</button>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="p-3 bg-zinc-900 border-t border-zinc-800 text-white flex flex-col space-y-1.5 z-10">
          ...
          {editingMessageId && (
            <div className="bg-zinc-950 p-2 rounded-xl flex gap-2 items-center border border-zinc-800">
              ...
              <button type="button" onClick={() => setEditingMessageId(null)} className="text-xs text-gray-500">취소</button>
            </div>
          )}
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 max-w-md w-full shadow-2xl">
            ...
            <div className="border-t border-zinc-800 pt-4 mb-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <button type="button" onClick={() => { setIsProfileModalOpen(false); setShowCapsuleModal(true); }} className="flex items-center gap-2 p-2.5 bg-zinc-950 border border-zinc-800 rounded-xl hover:bg-zinc-800 text-left">
                ...
                <button type="button" onClick={() => { setIsProfileModalOpen(false); setShowMemoryModal(true); }} className="flex items-center gap-2 p-2.5 bg-zinc-950 border border-zinc-800 rounded-xl hover:bg-zinc-800 text-left">
                ...
                <button type="button" onClick={() => { setIsProfileModalOpen(false); setShowScheduleModal(true); }} className="flex items-center gap-2 p-2.5 bg-zinc-950 border border-zinc-800 rounded-xl hover:bg-zinc-800 text-left">
                ...
                <button type="button" onClick={() => { setIsProfileModalOpen(false); setShowMemoModal(true); }} className="flex items-center gap-2 p-2.5 bg-zinc-950 border border-zinc-800 rounded-xl hover:bg-zinc-800 text-left">
            ...
            <div className="flex gap-2 mt-5 pt-2 border-t border-zinc-800">
              <button type="button" onClick={() => setIsProfileModalOpen(false)} className="w-1/3 bg-zinc-800 border border-zinc-700 py-2 rounded-xl text-xs font-bold hover:bg-zinc-700">닫기</button>
              <button type="button" onClick={handleSaveProfile} className="w-2/3 bg-white text-black py-2 rounded-xl text-xs font-bold hover:bg-gray-200">변경사항 저장</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 text-white">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 max-w-sm w-full">
            ...
            <div className="flex gap-2 mt-4">
              <button type="button" onClick={() => { setShowCapsuleModal(false); setIsProfileModalOpen(true); }} className="w-1/2 bg-zinc-800 border border-zinc-700 py-2 rounded-xl text-xs">이전으로</button>
      ...
      {showMemoryModal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 text-white">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 max-w-sm w-full">
            ...
            <button type="button" onClick={() => { setShowMemoryModal(false); setIsProfileModalOpen(true); }} className="w-full bg-zinc-800 border border-zinc-700 py-2 rounded-xl text-xs mt-4 font-bold hover:bg-zinc-700">이전으로</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 text-white">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 max-w-sm w-full">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => { setShowScheduleModal(false); setIsProfileModalOpen(true); }} className="w-1/2 bg-zinc-800 border border-zinc-700 py-2 rounded-xl text-xs">이전으로</button>
      ...
      {showMemoModal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 text-white">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 max-w-md w-full">
            ...
            <button type="button" onClick={() => { setShowMemoModal(false); setIsProfileModalOpen(true); }} className="w-full bg-zinc-800 border border-zinc-700 py-2 rounded-xl text-xs mt-4 font-bold hover:bg-zinc-700">이전으로</button>
          </div>
        </div>
      )}
